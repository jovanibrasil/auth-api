...
build: clean
	...
	docker build --build-arg USERS_MYSQL_URL --build-arg USERS_MYSQL_USERNAME --build-arg USERS_MYSQL_PASSWORD \
		 --build-arg RECAPTCHA_KEY_SITE --build-arg RECAPTCHA_KEY_SECRET --build-arg ENVIRONMENT=dev --network net -t auth-api .
